...
struct fast_rand {
  fast_rand(uint64_t seed0, uint64_t seed1) : s{seed0, seed1} {}
...
public:
  ...
  void wait() {
    std::unique_lock<std::mutex> s_lock(sem_mtx);
    sem_cv.wait(s_lock, [this]() { return sem_count > 0; });
    sem_count--;
  }
...
class barrier {
public:
  barrier(int count) : barrier_count(count) {}
  ...
  void wait() {
    ...
    if (barrier_count <= 0) {
      ...
      barrier_cv.wait(b_lock, [this]() { return barrier_count <= 0; });
    }
  }

...
public:
    ...
    {
        return creator_map.insert(std::pair<KeyType, VariantCreator>(id,
creator)).second;
    }
...
// assume that we won't need to manage any of the resources here (i.e. all data
// is stack-allocated and/or managed by other entities)
template <typename pixel_t> class flame_frame {
public:
  using T = pixel_t;

  flame_frame() : rows(0), cols(0), data(nullptr), manage_data(true) {}
  ...
  flame_frame(const flame_frame &other)
      : rows(other.rows), cols(other.cols), data(other.data),
        manage_data(other.manage_data) {}

  flame_frame(flame_frame &&other) : flame_frame() { swap(*this, other); }
  ...
  inline pixel_t *ptr(const int row) { return data + row * cols; }

  inline const pixel_t *ptr(const int row) const { return data + row * cols; }
...
template <typename pixel_t> struct histogram_info {
  histogram_info() : color{0, 0, 0} { frequency_count = 0; }

  histogram_info(pixel_t px_info, int freq_val) : color(px_info) {
...
template <typename data_t> struct flame_fcn_params {
  flame_fcn_params(data_t a = 0, data_t b = 0, data_t c = 0, data_t d = 0,
                   data_t e = 0, data_t f = 0)
...
template <typename data_t> struct flame_point {
  flame_point(const data_t row, const data_t col)
      : y(row), x(col), color{0.0, 0.0, 0.0}, param_a(0.0), param_b(0.0),
        param_c(0.0), param_d(0.0), param_e(0.0), param_f(0.0) {}
  ...
  // also store the last-applied affine variation parameters for the dependant
  // variations
...
public:
  ...
  flame_thread(flame_thread &&other) : rand_gen(other.rand_gen) {
  ...
  ~flame_thread() {
    // note: it's not a very good idea to do it this way. We would have to
    // interrupt the thread first. Plus I'm pretty sure join throws exceptions.
    // It'll be fine for the current usage, but not in any other
    if (fthread) {
  ...
  std::tuple<bool, std::thread::id> do_flame(fcn_t fcn, fcnargs_t &&... args) {
    ...
    fthread = std::unique_ptr<std::thread>(new std::thread(
        fcn, std::forward<fcnargs_t>(args)..., std::ref(rand_gen)));

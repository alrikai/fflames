...
public:
    ...
    void render(frame_t<pixel_t>* image, std::unique_ptr<std::vector<histogram_info<pixel_t>>> hist_data)
    {
        ...
        compute_rawimage_density<frame_t, pixel_t> (raw_image, freq_max_log, std::move(hist_data));
...
private:
    ...
    template <template <class> class frame_t, typename pixel_t = cv::Vec<data_t, 3>>
    void compute_rawimage_density (frame_t<pixel_t>& raw_image, const data_t freq_max_log, std::unique_ptr<std::vector<histogram_info<pixel_t>>> hist_data);

    template <template <class> class frame_t, typename pixel_t = cv::Vec<data_t, 3>>
    void render_frame_density (frame_t<pixel_t>&& raw_image, frame_t<pixel_t>* image);
...
void fflame_renderer<data_t>::compute_rawimage_density (frame_t<pixel_t>& raw_image, const data_t freq_max_log, std::unique_ptr<std::vector<histogram_info<pixel_t>>> hist_data)
{
    //reset the image density map for this frame
    image_density = cv::Mat_<uint64_t>::zeros(fflame_constants::imheight, fflame_constants::imwidth);
